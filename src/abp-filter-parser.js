...
function hasMatchingFilters (filterList, parsedFilterData, input, contextParams, cachedInputData, matchEmitter) {
  ...
  if (foundFilter && cachedInputData.matchedFilters && foundFilter.rawFilter) {
    ...
    // emit the match to the caller
    matchEmitter(input, foundFilter.rawFilter)
  }
  return !!foundFilter
}
 ...
 */
export function matches (parserData, input, contextParams = {}, cachedInputData = { }, matchEmitter = (site, rule) => {}) {
  ...
  cachedInputData.matchedFilters = cachedInputData.matchedFilters || new Set()
  ...
  if (parserData.bloomFilter) {
    if (!parserData.bloomFilter.substringExists(cleanedInput, fingerprintSize)) {
      ...
      hasMatchingNoFingerprintFilters =
          hasMatchingFilters(parserData.noFingerprintFilters, parserData, input, contextParams, cachedInputData, matchEmitter)
  ...
  if (hasMatchingFilters(parserData.filters, parserData, input, contextParams, cachedInputData, matchEmitter) ||
    ...
    if (!exceptionBloomFilterMiss || hasMatchingFilters(parserData.exceptionFilters, parserData, input, contextParams, cachedInputData, matchEmitter)) {
      cachedInputData.notMatchCount++
      return false
    }
    return true
  ...
  // The bloom filter had a false positive, so we checked for nothing! :'(
